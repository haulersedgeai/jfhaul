import Image from "next/image";
import Link from "next/link";
import { Phone, ArrowUpRight, Check, Star } from "lucide-react";
import {
  birminghamPathForService,
  business,
  faqs,
  howItWorks,
  services,
  type ServiceArea,
} from "@/data/site";
import { iconFor } from "@/data/serviceIcons";
import { Section, SectionHeader } from "@/components/ui/Section";
import { Card } from "@/components/ui/Card";
import { Kicker } from "@/components/ui/Kicker";
import { Reveal } from "@/components/site/Reveal";
import { QuoteForm } from "@/components/site/QuoteForm";
import { FAQAccordion } from "@/components/site/FAQAccordion";
import { FAQSchema, ServiceSchema } from "@/components/site/StructuredData";
import { TrustBar } from "@/components/site/TrustBar";
import { ReviewCard } from "@/components/site/ReviewCard";
import { pickReviewsFor, reviewStats } from "@/data/reviews";

/**
 * Surface-level hub for a single town within J&F's 37-mile service radius
 * around Calera, AL. Kept intentionally light; we'll add per-service pages
 * for these towns later.
 */
export function TownHub({ area }: { area: ServiceArea }) {
  const cityState = `${area.name}, ${area.state}`;
  const canonical = `${business.siteUrl}/junk-removal-${area.slug}-al`;
  const localReviews = pickReviewsFor({ city: area.name, category: "Junk Removal", limit: 2 });

  const townFaqs = [
    {
      q: `Do you actually service ${area.name}, AL?`,
      a: `Yes — ${area.name} is well inside our 37-mile service radius from Calera. Same-day is often possible when a truck is rolling nearby.`,
    },
    {
      q: `How much does junk removal cost in ${area.name}?`,
      a: `We price by how much space your items take up in the truck, not by the hour. You'll get a clear, upfront quote in ${area.name} before we start.`,
    },
    ...faqs.slice(1, 4),
  ];

  return (
    <>
      <FAQSchema items={townFaqs} />
      <ServiceSchema
        serviceName="Junk Removal"
        areaServed={cityState}
        url={canonical}
        description={`Junk removal in ${cityState} by ${business.legalName}. Family + woman-owned. Same-day when possible.`}
      />

      {/* HERO */}
      <section className="relative overflow-hidden bg-gradient-to-b from-cream-50 to-white">
        <div className="hero-arch" aria-hidden="true" />
        <div className="grain absolute inset-0" aria-hidden="true" />
        <div className="container-x pt-10 md:pt-14 pb-12 md:pb-16 relative">
          <nav aria-label="Breadcrumb" className="text-sm text-ink-400 mb-4">
            <Link href="/" className="hover:text-brand-800">Home</Link>
            <span aria-hidden="true"> · </span>
            <Link href="/service-areas" className="hover:text-brand-800">Service areas</Link>
            <span aria-hidden="true"> · </span>
            <span className="text-ink-700">{cityState}</span>
          </nav>
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] items-center">
            <Reveal eager>
              <Kicker>Serving {cityState}</Kicker>
              <h1 className="mt-4 font-display font-black tracking-[-0.025em] text-ink-800 leading-[1.02] [text-wrap:balance] text-[clamp(2.5rem,6vw,4.5rem)]">
                Junk Removal in <span className="text-brand-700">{cityState}</span>
              </h1>
              <p className="mt-5 text-lg md:text-xl text-ink-500 max-w-xl [text-wrap:pretty]">
                {area.name} sits well inside J&F's 37-mile service radius from Calera, AL — so whether you're clearing out a garage, a rental, or a full estate, we can usually get there fast. Same-day service when we can, always with a free upfront quote.
              </p>
              <p className="mt-4 text-ink-500 max-w-xl">
                Licensed, insured, family + woman-owned. Jacorie, Felicia, and the crew do the heavy lifting — you just point.
              </p>
              <div className="mt-7 flex flex-wrap items-center gap-3">
                <Link
                  href="#quote"
                  className="inline-flex items-center gap-2 rounded-full bg-accent-500 hover:bg-accent-600 text-white font-semibold px-6 py-4 text-lg shadow-[var(--shadow-lift)]"
                >
                  Get a free quote
                  <ArrowUpRight size={18} aria-hidden="true" />
                </Link>
                <a
                  href={business.telHref}
                  className="inline-flex items-center gap-2 rounded-full bg-white border border-brand-100 hover:border-brand-300 text-brand-800 px-6 py-4 text-lg font-semibold"
                >
                  <Phone size={18} aria-hidden="true" /> Call {business.phone}
                </a>
              </div>
            </Reveal>
            <Reveal eager delay={0.1}>
              <div className="relative">
                <div className="absolute -inset-x-4 -inset-y-6 rounded-[36px] bg-gradient-to-br from-brand-100 via-cream-200 to-accent-100 opacity-70 blur-2xl" aria-hidden="true" />
                <div className="absolute -bottom-4 -right-4 h-2/3 w-3/4 rounded-[36px] bg-accent-500/95" aria-hidden="true" />
                <div className="relative overflow-hidden rounded-[28px] shadow-[var(--shadow-lift)] border border-white">
                  <Image
                    src="/images/hero.png"
                    alt={`J&F Haul crew working in ${cityState}`}
                    width={1200}
                    height={800}
                    loading="eager"
                    fetchPriority="high"
                    sizes="(min-width: 1024px) 560px, 100vw"
                    className="w-full h-auto"
                  />
                </div>
                <a
                  href={reviewStats.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hidden md:flex absolute -bottom-6 -left-6 rounded-2xl bg-white shadow-[var(--shadow-lift)] p-4 items-center gap-3 max-w-xs"
                  aria-label={`${reviewStats.rating} stars based on ${reviewStats.count} Google reviews`}
                >
                  <div className="h-11 w-11 rounded-full bg-accent-50 grid place-items-center text-accent-600 shrink-0">
                    <Star size={20} aria-hidden="true" fill="currentColor" />
                  </div>
                  <div>
                    <div className="flex text-accent-500" aria-hidden="true">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} size={14} fill="currentColor" strokeWidth={0} />
                      ))}
                    </div>
                    <div className="mt-1 text-sm font-bold text-ink-800">
                      {reviewStats.rating.toFixed(1)} · {reviewStats.count} Google reviews
                    </div>
                  </div>
                </a>
              </div>
            </Reveal>
          </div>
        </div>
      </section>

      <TrustBar />

      {/* SERVICES OFFERED */}
      <Section bg="white">
        <SectionHeader
          eyebrow={`What we haul in ${area.name}`}
          title="Every kind of junk. One friendly crew."
          intro={`Everything on this list is available in ${cityState}. Not sure? Send us a photo.`}
        />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => {
            const Icon = iconFor(s.slug);
            return (
              <Card key={s.slug} className="p-5 h-full flex items-start gap-4" as="article">
                <div className="h-11 w-11 rounded-2xl bg-brand-50 text-brand-700 grid place-items-center shrink-0">
                  <Icon size={22} strokeWidth={2.1} aria-hidden="true" />
                </div>
                <div className="flex-1 min-w-0">
                  <Link
                    href={birminghamPathForService(s.slug)}
                    className="font-semibold text-ink-800 hover:text-brand-800"
                  >
                    {s.name}
                  </Link>
                  <p className="text-sm text-ink-500 mt-1">{s.blurb}</p>
                </div>
              </Card>
            );
          })}
        </div>
        <p className="mt-6 text-sm text-ink-500 text-center">
          Booking any of these in {area.name}? Same phone number:
          {" "}
          <a href={business.telHref} className="font-semibold text-brand-800 hover:text-brand-900">
            {business.phone}
          </a>.
        </p>
      </Section>

      {/* QUOTE FORM */}
      <Section id="quote" bg="cream" padded>
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] items-start">
          <Reveal>
            <SectionHeader
              eyebrow={`${area.name}, AL`}
              title="Free upfront quote. Usually text-fast."
              intro={`Tell us what needs to go and we'll send a quote for ${area.name}. Photos welcome.`}
            />
            <div className="rounded-3xl bg-white p-6 shadow-[var(--shadow-soft)]">
              <a href={business.telHref} className="text-3xl font-bold text-brand-800 hover:text-brand-900 tracking-tight">
                {business.phone}
              </a>
              <div className="mt-2 text-sm text-ink-500">{business.hours}</div>
              <div className="mt-4 flex gap-2 flex-wrap">
                <a href={business.telHref} className="inline-flex items-center gap-2 rounded-full bg-accent-500 hover:bg-accent-600 text-white px-4 py-2 text-sm font-semibold">
                  <Phone size={14} aria-hidden="true" /> Call
                </a>
                <a href={business.smsHref} className="inline-flex items-center gap-2 rounded-full bg-brand-700 hover:bg-brand-800 text-white px-4 py-2 text-sm font-semibold">
                  Text
                </a>
              </div>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <QuoteForm
              heading={`Get a ${area.name} quote`}
              defaultCity={area.name}
              source={`town:${area.slug}`}
            />
          </Reveal>
        </div>
      </Section>

      {/* HOW IT WORKS */}
      <section className="bg-brand-800 text-white relative overflow-hidden">
        <div className="container-x py-16 md:py-20 relative">
          <SectionHeader
            eyebrow="How it works"
            title={`Booking us in ${area.name} is simple.`}
            centered
            tone="dark"
          />
          <ol className="grid gap-5 md:grid-cols-3 lg:grid-cols-6 steps-connector">
            {howItWorks.map((step, i) => (
              <Reveal key={step.title} delay={Math.min(i * 0.05, 0.3)} as="li">
                <div className="relative z-10 rounded-3xl bg-white/[0.05] backdrop-blur border border-white/10 p-6 h-full">
                  <div className="h-11 w-11 rounded-full bg-accent-500 text-white font-black grid place-items-center text-lg mb-4 shadow-[0_10px_20px_rgba(225,45,36,0.35)]">
                    {i + 1}
                  </div>
                  <div className="font-semibold text-white">{step.title}</div>
                  <div className="text-sm text-white/70 mt-1">{step.body}</div>
                </div>
              </Reveal>
            ))}
          </ol>
        </div>
      </section>

      {/* REVIEWS */}
      <Section bg="white">
        <div className="grid gap-8 md:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)] items-start">
          <Reveal>
            <Kicker>Reviews</Kicker>
            <div className="mt-3 flex items-center gap-3">
              <div className="font-display font-black text-ink-800 text-5xl leading-none">
                {reviewStats.rating.toFixed(1)}
              </div>
              <div>
                <div className="flex text-accent-500" aria-hidden="true">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={14} fill="currentColor" strokeWidth={0} />
                  ))}
                </div>
                <div className="text-sm text-ink-500">{reviewStats.count} Google reviews</div>
              </div>
            </div>
            <p className="mt-4 text-ink-500 max-w-md">
              A few honest words from customers around Birmingham and the surrounding towns.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              <a
                href={reviewStats.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-full bg-accent-500 hover:bg-accent-600 text-white px-4 py-2 text-sm font-semibold"
              >
                Read on Google
                <ArrowUpRight size={14} aria-hidden="true" />
              </a>
              <Link
                href="/reviews"
                className="inline-flex items-center gap-2 rounded-full bg-white border border-ink-100 hover:border-brand-300 px-4 py-2 text-sm font-semibold text-brand-800"
              >
                All reviews
              </Link>
            </div>
          </Reveal>
          <div className="grid gap-5 sm:grid-cols-2">
            {localReviews.map((r) => (
              <Reveal key={r.name}>
                <ReviewCard review={r} />
              </Reveal>
            ))}
          </div>
        </div>
      </Section>

      {/* CITY-SPECIFIC CTA */}
      <section className="bg-gradient-to-b from-cream-100 to-white">
        <div className="container-x py-14 md:py-20">
          <div className="grid gap-8 md:grid-cols-2 items-center">
            <Reveal>
              <Kicker>Local & fast</Kicker>
              <h2 className="mt-4 font-display font-black tracking-[-0.02em] text-ink-800 leading-[1.05] text-[clamp(2rem,4.5vw,3rem)]">
                Need us in {area.name} today?
              </h2>
              <p className="mt-3 text-ink-500 max-w-xl text-lg">
                Call or text {business.phone}. Same-day is often possible when a J&F truck is rolling through the {area.name} area.
              </p>
              <ul className="mt-4 space-y-2 text-ink-500">
                {["37-mile radius from Calera, AL", "Licensed & insured", "Free upfront quotes"].map((t) => (
                  <li key={t} className="flex items-center gap-2">
                    <Check size={16} className="text-accent-500" aria-hidden="true" />
                    {t}
                  </li>
                ))}
              </ul>
            </Reveal>
            <Reveal delay={0.1} className="md:justify-self-end">
              <div className="flex flex-wrap gap-3">
                <a
                  href={business.telHref}
                  className="inline-flex items-center gap-2 rounded-full bg-accent-500 hover:bg-accent-600 text-white px-6 py-4 text-lg font-semibold shadow-[var(--shadow-lift)]"
                >
                  <Phone size={18} aria-hidden="true" /> Call {business.phone}
                </a>
                <a
                  href={business.smsHref}
                  className="inline-flex items-center gap-2 rounded-full bg-brand-700 hover:bg-brand-800 text-white px-6 py-4 text-lg font-semibold"
                >
                  Text us
                </a>
              </div>
            </Reveal>
          </div>
        </div>
      </section>

      {/* FAQ */}
      <Section bg="white">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] items-start">
          <Reveal>
            <SectionHeader
              eyebrow="FAQ"
              title={`${area.name} junk removal questions`}
              intro={`Common questions from folks calling us from ${area.name} for the first time.`}
            />
          </Reveal>
          <Reveal delay={0.1}>
            <FAQAccordion items={townFaqs} />
          </Reveal>
        </div>
      </Section>
    </>
  );
}
